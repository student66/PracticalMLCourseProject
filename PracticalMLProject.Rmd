...
### Study design
 ...
 - Partition the given training set into three sub-sets: 
     ...
     - validation set
     - testing set
 - Build several models using the trainig set.
 - Use the validataion set to compate the models and select one of them as final.
...
Once the selected method is ealuated, it will be used to make predictions on the given test set and submit the results.
...
Most features do not show strong correlations. The decison is to keep them all for now.
...
GBM shows several almost unimportant features. The RF method shows all vars as more or less important. For now I am keeping all variables.

### Modeling

Using the `caret` library, I wil lbuild and compare three models of different kinds - GBM, Random Forest, and SVM.
...
The followwing code can be used to print and review the full confusion matrices.
...
The following code runs the previously created and selected GBM model on the *testing set*. Now for the first time I am using the *testing set*. 
...
The following code below is used to read the 20-sample dataset,  select the features used on the training set, and then predict the outcome using the previously built GBM model. Note that the 20-sample test does not have outcome variable. It wil lbe predicted and submitted to Coursera for grading.
